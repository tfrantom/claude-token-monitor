...
const { renderLine, fmtK, fmtCostShort, isSnapshotFresh } = require('./statusline');
...
check('usage is counted once when a tool_result interleaves two same-id lines', () => {
  const p = fixture('parallel-tools.jsonl', [
    { type: 'user', timestamp: '2026-08-07T12:00:00Z', message: { content: 'do two things at once please' } },
    { type: 'assistant', timestamp: '2026-08-07T12:00:01Z', message: { id: 'm1', model: 'claude-opus-5', usage: ASSISTANT_USAGE, content: [{ type: 'tool_use', id: 'tu1', name: 'Read', input: { file: 'a' } }] } },
    { type: 'user', timestamp: '2026-08-07T12:00:02Z', message: { content: [{ type: 'tool_result', tool_use_id: 'tu1', content: 'ok' }] } },
    { type: 'assistant', timestamp: '2026-08-07T12:00:03Z', message: { id: 'm1', model: 'claude-opus-5', usage: ASSISTANT_USAGE, content: [{ type: 'tool_use', id: 'tu2', name: 'Read', input: { file: 'b' } }] } },
    { type: 'user', timestamp: '2026-08-07T12:00:04Z', message: { content: [{ type: 'tool_result', tool_use_id: 'tu2', content: 'ok' }] } },
  ]);
  const t = classifySession(p).totals;
  assert.strictEqual(t.context, 100, 'input_tokens must not be double-counted');
  assert.strictEqual(t.cache_read, 500);
  assert.strictEqual(t.cache_write, 200);
  const prorated = t.thinking + t.writing + t.tool_calls;
  assert.ok(Math.abs(prorated - 1000) < 1e-6, `output summed to ${prorated}, expected the turn's 1000`);
});

check('a turn resumed after an interleaved line keeps all of its blocks', () => {
  const p = fixture('parallel-blocks.jsonl', [
    { type: 'user', timestamp: '2026-08-07T12:00:00Z', message: { content: 'spawn two agents for me' } },
    { type: 'assistant', timestamp: '2026-08-07T12:00:01Z', message: { id: 'm1', model: 'claude-opus-5', usage: ASSISTANT_USAGE, content: [{ type: 'tool_use', id: 'a1', name: 'Agent', input: { description: 'first' } }] } },
    { type: 'user', timestamp: '2026-08-07T12:00:02Z', message: { content: [{ type: 'tool_result', tool_use_id: 'a1', content: 'launched' }] } },
    { type: 'assistant', timestamp: '2026-08-07T12:00:03Z', message: { id: 'm1', model: 'claude-opus-5', usage: ASSISTANT_USAGE, content: [{ type: 'tool_use', id: 'a2', name: 'Agent', input: { description: 'second' } }] } },
  ]);
  const r = classifySession(p);
  assert.deepStrictEqual(r.agentUses.map((a) => a.description), ['first', 'second']);
  assert.strictEqual(r.turns.length, 1, 'one message.id is one turn, however many lines it spans');
  assert.strictEqual(r.turns[0].blocks.length, 2);
});

check('assistant entries with no message.id are not merged into one turn', () => {
  const p = fixture('no-id.jsonl', [
    { type: 'user', timestamp: '2026-08-07T12:00:00Z', message: { content: 'a real user message here' } },
    { type: 'assistant', timestamp: '2026-08-07T12:00:01Z', message: { model: 'claude-opus-5', usage: ASSISTANT_USAGE, content: [{ type: 'text', text: 'one' }] } },
    { type: 'assistant', timestamp: '2026-08-07T12:00:02Z', message: { model: 'claude-opus-5', usage: ASSISTANT_USAGE, content: [{ type: 'text', text: 'two' }] } },
  ]);
  const t = classifySession(p).totals;
  assert.strictEqual(t.context, 200, 'two id-less entries are two turns, not one');
});
...
check('a tag-wrapped local-command preamble is not a user message', () => {
  const p = fixture('caveat.jsonl', [
    { type: 'user', timestamp: '2026-09-08T12:00:00Z', message: { content: '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to.</local-command-caveat>' } },
    { type: 'user', timestamp: '2026-09-08T12:00:01Z', message: { content: '<command-name>/login</command-name>\n            <command-message>login</command-message>\n            <command-args></command-args>' } },
    { type: 'user', timestamp: '2026-09-08T12:00:02Z', message: { content: '<local-command-stdout>Login successful</local-command-stdout>' } },
    { type: 'user', timestamp: '2026-09-08T12:00:03Z', message: { content: 'now fix the thing I actually asked about' } },
  ]);
  assert.deepStrictEqual(classifySession(p).userTexts, ['now fix the thing I actually asked about']);
});

check('real prose after a command preamble in the same turn survives', () => {
  const p = fixture('caveat-mixed.jsonl', [
    { type: 'user', timestamp: '2026-09-08T12:00:00Z', message: { content: '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to.</local-command-caveat>the brief the human actually typed' } },
    { type: 'user', timestamp: '2026-09-08T12:00:01Z', message: { content: '<command-name>/go</command-name><command-args>x</command-args> and then do the work' } },
  ]);
  assert.deepStrictEqual(classifySession(p).userTexts, [
    'the brief the human actually typed',
...
check('a stale snapshot renders the watcher state, not its frozen sessions', () => {
  const stale = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  const out = strip(renderLine({ session_id: 's1' }, { updated_at: stale, sessions: { s1: session() } }, 'disabled'));
  assert.ok(!out.includes('Test Session'), `a dead watcher's last snapshot must not render as live, got: ${out}`);
  assert.ok(out.includes('autostart disabled'), `expected the watcher state, got: ${out}`);
});

check('a snapshot with no updated_at is not treated as fresh', () => {
  assert.ok(!isSnapshotFresh({ sessions: {} }));
  assert.ok(!isSnapshotFresh({ updated_at: '', sessions: {} }));
  assert.ok(isSnapshotFresh({ updated_at: new Date().toISOString() }));
});

check('an empty session set renders a message, not a crash', () => {
  assert.ok(strip(renderLine({}, { updated_at: new Date().toISOString(), sessions: {} })).includes('no active sessions'));
});
...
const { buildActivity, isRegistryEntryLive } = require('./watcher');
const { activityMark, ACTIVITY } = require('./statusline');

const MIN = 60 * 1000;

check('a registry entry from before the last boot is never live', () => {
  const boot = Date.now() - 60 * MIN;
  const ours = { sessionId: 's1', pid: process.pid };
  assert.ok(isRegistryEntryLive({ ...ours, startedAt: boot + 30 * MIN }, boot), 'a live pid started this boot is live');
  assert.ok(!isRegistryEntryLive({ ...ours, startedAt: boot - 30 * MIN }, boot), 'a recycled pid must not resurrect a pre-reboot session');
  assert.ok(isRegistryEntryLive({ ...ours }, boot), 'an entry with no startedAt falls back to the pid probe');
  assert.ok(!isRegistryEntryLive({ sessionId: 's1', pid: 0x7fffffff, startedAt: boot + 30 * MIN }, boot), 'a dead pid is not live');
});
